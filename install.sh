...
arch-chroot /mnt bash -c "pacman -Sy archlinuxcn-keyring --noconfirm;hwclock --systohc;locale-gen;pacman -S nerd-fonts-fira-code grub efibootmgr --noconfirm;mv /root/grub /etc/default/grub;grub-install --target=x86_64-efi --efi-directory=/efi --bootloader-id=ArchLinux;grub-mkconfig -o /boot/grub/grub.cfg;systemctl enable dhcpcd iwd;useradd -m -G wheel -s /bin/bash nobume"
exit 0
