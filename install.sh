...
arch-chroot /mnt bash -c "pacman -Sy archlinuxcn-keyring --noconfirm;pacman -S nerd-fonts-fira-code fcitx5-pinyin-moegirl lib32-mesa lib32-vulkan-intel lib32-nvidia-utils --noconfirm;hwclock --systohc;locale-gen;mv /root/grub /etc/default/grub;grub-install --target=x86_64-efi --efi-directory=/efi --bootloader-id=ArchLinux;grub-mkconfig -o /boot/grub/grub.cfg;systemctl enable dhcpcd iwd;useradd -m -G wheel -s /bin/bash nobume"
exit 0
